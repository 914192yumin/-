...
if uploaded_file is not None:
    try:
        ...
        st.error(f"讀取失敗：請確認檔案中是否有名為「醫師總表」的分頁。詳細錯誤：{e}")
...
if st.session_state.doc_database is not None:
    ...
    # 建立醫師 MVPN 對應字典，方便最後產出班表時使用
    # 將 MVPN 轉為字串並去除小數點 (處理 Excel 讀取數字時可能變浮點數的問題)
    mvpn_dict = {}
    ...
    with tab2:
        ...
        if st.button("產生最終班表", type="primary"):
            ...
            for d, doc in schedule.items():
                if doc is None:
                    if regular_second_line: schedule[d] = random.choice(regular_second_line)
                    else: schedule[d] = "待補"
            ...
            st.download_button(label="📥 下載最終班表 (CSV)", data=csv, file_name='final_schedule_with_mvpn.csv', mime='text/csv')
